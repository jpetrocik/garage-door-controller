...
{
  if (btn.wasPressedFor() > 4000)
  {
    deviceConfig.startTicker(0.25);
  }
  else if (btn.wasPressedFor() > 2000)
  {
...
void setup()
{
  ...
  button.setReleasedHandler(buttonReleasedHandler);
  button.setLongClickDetectedRetriggerable(true);
  button.setLongClickDetectedRetriggerable(longPressButtonHandler);
  button.setLongClickDetectedHandler(longReleaseButtonHandler);
